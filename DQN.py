...
def get_actions_of_state(adj_matrix, state):
    ...
    for next_s in np.argwhere(adj_matrix[np.argmax(state)] > 0):
        for vehicle in range(3):
            ...
            act[next_s] = 1
            # act[num_nodes+vehicle] = 1 ## just a test
            act[num_nodes+vehicle] = vehicle+1
            actions.append(act)
    return actions
...
def function_approximation_Egreedy(env, episodes):
    ...
    for episode in tqdm(range(episodes)):
        ...
        while not done and steps < 60:
            ...
            state = next_state
            steps += 1
            epsilon = max(0.1, epsilon * 0.995) # Shrinking the exploration rate during time
    return model
...
if __name__ == "__main__":
    ...
    states = np.zeros((num_nodes, num_nodes), dtype=np.float32)
    ...
    for s in states:
        ...
        print(ascii_uppercase[ np.argmax(s) ], "--->",
            ...
            "way is ", np.argmax(best_action[-3:]),
            f"Q(s, a) = { max_q }")
        
    # s = np.zeros((num_nodes), dtype=np.float32)
    # s[np.random.randint(14)] = 1 

    # g = np.zeros((num_nodes), dtype=np.float32)
    # g[-1] = 1 
